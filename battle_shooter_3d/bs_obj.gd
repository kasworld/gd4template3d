...
func init_shield(src_ship :BSObj) -> BSObj:
	...
	animation.start_scale("shield_spawn", $MeshInstance3D, Vector3(0.1,0.1,0.1), Vector3(1,1,1), AniDurSec)
...
func init_bullet(t_num :int, velocity_a :Vector3) -> BSObj:
	...
	animation.start_scale("bullet_spawn", $MeshInstance3D, Vector3(0.1,0.1,0.1), Vector3(1,1,1), AniDurSec)
	return self
...
func init_homming(t_num :int, dstobj :BSObj) -> BSObj:
	...
	animation.start_scale("homming_spawn", $MeshInstance3D, Vector3(0.1,0.1,0.1), Vector3(1,1,1), AniDurSec)
	return self
...
func end_life(other :BSObj) -> void:
	...
	match type:
		Type.Ship:
			animation.start_scale("ship_explode", $MeshInstance3D, Vector3(1,1,1), Vector3(0.1,0.1,0.1), AniDurSec)
		Type.Shield:
			animation.start_scale("shield_explode", $MeshInstance3D, Vector3(1,1,1), Vector3(0.1,0.1,0.1), AniDurSec)
		Type.Bullet:
			animation.start_scale("bullet_explode", $MeshInstance3D, Vector3(1,1,1), Vector3(0.1,0.1,0.1), AniDurSec)
		Type.Homming:
...
func ship_ai_act(delta: float, game :BattleShooter) -> void:
	var node_list := game.octree.search(position, game.octree_search_size)
