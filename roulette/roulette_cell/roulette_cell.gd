...
func init(각도 :float, 반지름 :float, 깊이 :float, color_text_info :Array, text_size :float = 0.0 ) -> RouletteCell:
	$"글씨".mesh.depth = 깊이
	$"글씨".mesh.pixel_size = 반지름 *sin(각도) *0.04 if text_size == 0.0 else text_size
